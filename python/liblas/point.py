...
class Point(object):
    def __init__(self, owned=True, handle=None, copy=False):
        ...
        :param owned: boolean
            A flag to denote whether or not the point owns itself
            and can destroy itself upon going out of scope.  There 
            are instances where :obj:`liblas.file.File` may actually 
            own the point as it is read from or written to files.
        
        :param handle: ctypes object reference
            A handle to an existing ctypes LASPointH object reference.  
            Only internal liblas code should have to worry about using 
            this.
        
        :param copy: boolean
            A flag to denote whether or not to copy the point upon 
            creation.  Used in coordination with the :obj:`handle` 
    ...
    """    
    number_of_returns = property(get_number_of_returns, set_number_of_returns, None, doc)
    ...
    """
    scan_direction = property(get_scan_direction, set_scan_direction, None, doc)
    ...
    """
    flightline_edge = property(get_flightline_edge, set_flightline_edge, None, doc)
    ...
    doc = """Scan flags for the point.  This is a combination of the :obj:`flightline_edge` \
    :obj:`return_number` :obj:`number_of_returns` and :obj:`scan_direction`.  Use the 
    individual methods for setting these values for convenience.
    """
    ...
    doc = """The point's classification as a raw byte value.
        ...
        The following large section of information is verboten from the specification_ 
        ...
        .. note::
            Bits 5, 6 and 7 are treated as flags and can be set or
            clear in any combination. For example, a point with bits 5 and 6
            both set to one and the lower five bits set to 2 (see table below)
            would be a ground point that had been Synthetically collected and
            marked as a model key-point.
    ...
    """
    classification = property(get_classification, set_classification, None, doc)
    ...
    doc = """Interpeted (:class:`datetime.datetime` instance) time value for the point.
        ...
        .. note::
            ...
            Because no header is available, no coordination 
            is done with respect to the allowable LAS time types.  You 
            must coordinate with the :obj:`liblas.header.Header.global_encoding` 
            to determine the actual time value for the point.  If you want 
            to merely preserve existing point data, use 
            :obj:`liblas.point.Point.raw_time`
    ...
    doc = """Uninterpeted time value for the point.
        ...
        .. note::
            ...
            is done with respect to the allowable LAS time types.  You 
            must coordinate with the :obj:`liblas.header.Header.global_encoding` 
            to determine the actual time value for the point.  See the ASPRS 
            LAS 1.1-1.3 specifications for more detail.
        """
    ...
    doc = """Raw color value for the point as an :obj:`liblas.color.Color` instance.
    ...
    of extra data to each individual point by setting the :obj:`liblas.header.Header.dataformat_id` 
    to a valid value, and then setting the :obj:`liblas.header.Header.data_record_length` to 
    a value that is larger than the nominal length for that format as noted in the specification_.
    ...
    .. note::
        The data will be clipped to exactly the :obj:`liblas.header.Header.data_record_length` 
        when they are actually written to the file.  If you set data on the point 
        that is larger than the difference between the nominal point format 
        data record length and the data record length you set in the header, it 
        will be thrown away.  
